...
| `app/api/routes/sb/files.py` | Router | `GET /sb/files`, `POST /sb/files/upload`, `PATCH /sb/files/{id}/link`, `DELETE /sb/files/{id}` |
...
```bash
# All files
curl "http://localhost:8000/sb/files"

# Files linked to a specific project
curl "http://localhost:8000/sb/files?project_id=abc-123"

# Design department assets only
curl "http://localhost:8000/sb/files?department=Design"
```
...
**curl example:**
```bash
curl -X POST http://localhost:8000/sb/files/upload \
...
const res = await fetch("/sb/files/upload", { method: "POST", body: form });
const data = await res.json();
```

**Response `201`:** The created file metadata record (same shape as `GET /sb/files` items).
...
**Test:**
```bash
curl -X PATCH http://localhost:8000/sb/files/file-uuid-.../link \
  -H "Content-Type: application/json" \
...
**Test:**
```bash
curl -X DELETE http://localhost:8000/sb/files/file-uuid-...
```
...
│  Asset list (Design)       →  GET /sb/files?department=Design      │
│  Asset upload              →  POST /sb/files/upload (multipart)    │
│  Link asset to project     →  PATCH /sb/files/{id}/link            │
│  Delete asset              →  DELETE /sb/files/{id}                │
...
POST /sb/files/upload   (multipart/form-data)
...
### `POST /sb/files/upload` returns `500 Upload failed`
